...
## From: lock-objects-during-move (2026-07-14)

- **Concurrent duplicate move requests can silently steal `MOVE_LOCK` and double-queue a paste**: `MOVE_LOCK` is `stealable=True` (matching every other custom lock type in `opengever.locking.lock`), and neither `MoveItemsForm.handle_submit` nor `Move.reply` checks `ILockable(obj).locked(MOVE_LOCK)` before locking. Two near-simultaneous move requests for the same object each successfully cut and lock it — the second silently overwrites the first's lock and queues a second `move-objects` task. This race already existed before this change (with zero synchronization at all), so this story doesn't introduce it, only narrows it; but it's not fully closed either. Whichever task's `paste_clipboard()` runs second will most likely hit an already-consumed source path and get swallowed as an expected `CopyError`/`ValueError`, and whichever task's `finally` runs last unlocks the object regardless of which task's lock is "current" (no ownership check on unlock). Closing this fully requires a product decision — reject the second concurrent move outright (e.g. raise `ResourceLockedError` if `ILockable(obj).locked(MOVE_LOCK)` before locking) vs. accept the narrow race as-is — which is out of scope of the literal ask ("lock while queued, clear when the task finishes").

## From spec-background-tasks-control-panel review (2026-09-04)

- **`FilterList.update_query` raises `KeyError` on an unknown filter id.** `opengever/tabbedview/filters.py:93` does `self[selected_filter_id].update_query(query)` without a membership check, so a crafted or stale `?<filterlist_name>=...` value yields a 500 on *any* tabbedview listing, not just the background tasks panel. Pre-existing and shared by every filtered tab, so fixing it belongs in its own change.
- **SQL listings never push batching down into the query.** `BaseListingTab.lazy = False` (`opengever/tabbedview/browser/base_tabs.py:191`) makes `SqlTableSource.extend_query_with_batching` return early, so the full result set is loaded per request and batched in Python. Harmless for the bounded OGDS user list; a real risk for `background_tasks`, which has no retention policy and can reach millions of rows. The inline comment there ("lazy must be false otherwise there will be no correct batching") contradicts `sqlsource.py`, so this needs investigation rather than a flag flip.
- **No row cap or time window on the background tasks listing.** Combined with the point above, `filter_all` on a large table means a full scan plus a `COUNT(*)` per request. A default "created within the last N days" restriction would bound it, but that changes the spec'd behaviour ("all background tasks"), so it needs a product decision.
- **The background tasks configlet ignores the `bgtasks` feature flag.** `condition_expr=""` in `opengever/core/profiles/default/controlpanel.xml`, and neither the control panel nor the listing consults `is_background_tasks_enabled()`. A site with the feature off still shows the panel with a permanently empty table. project-context.md says "Never hardcode feature availability - always check the registry flag."
- **The extjs rendering path is untested for tabbedview listings.** `IntegrationTestCase` deactivates extjs, but production runs with it, and `generic_with_filters.pt` only renders the filters-on-empty-result block inside its `view/extjs_enabled` branch. The behaviour that motivated using `FilteredListingTab` is therefore not covered by an automated test.
