...
- **Existing installations need an upgrade step for the new registry record**: `opengever.bgtasks.interfaces.IBackgroundTaskSettings` is only registered via `opengever/core/profiles/default/registry.xml`, which GenericSetup only auto-applies on fresh installs. Sites that already exist and don't reinstall/upgrade that step will hit `is_background_tasks_enabled()`'s fail-safe `except` branch (record not found) and stay permanently disabled — the opposite of the declared `default=True` — with no indication why. Needs an upgrade step (in `opengever/core/upgrades/`) importing the new record, following the same pattern used for other feature-flag registry records in that package. Not addressed here because `opengever/core/upgrades/**` is read-denied in this session's permission settings, and guessing the next sequential upgrade-step number without verifying the existing chain risks corrupting it.
