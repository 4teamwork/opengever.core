...
class TestProposal(IntegrationTestCase):
    ...
    def test_creating_proposal_from_tabbedview_sets_attachments(self, browser):
        ...
        document_36_path = ('paths:list', browser.css('#document-36').first.node.attrib.get('value'), )
        method = ('++add++opengever.meeting.proposal:method', '1', )
        browser.open(
            self.dossier.absolute_url(),
            headers={'Content-Type': 'application/x-www-form-urlencoded'},
            data=formdata.urlencode((original_template, authenticator, document_14_path, document_36_path, method, )),
            )
        self.assertEqual(
