...
class DossierContainer(Container):
    ...
    def earliest_possible_end_date(self):
        """The earliest possible end-date must be after all document dates and
        ...
        for dossier_brain in dossier_brains:
            end_dates.append(dossier_brain.end)
            end_dates.append(dossier_brain.start)
        ...
        end_dates.extend(
            [document_brain.document_date for document_brain in document_brains])

        end_dates = filter(None, end_dates)
