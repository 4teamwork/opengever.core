...
class TestBusinessCaseDossierIntegration(unittest.TestCase):
    ...
    def test_overview(self):
        ...
        provideUtility(
            DummyContactVocabulary(), name='opengever.ogds.base.ContactsVocabulary')
        provideUtility(
            DummyUsersVocabulary(), name='opengever.ogds.base.UsersVocabulary')
        ...
        subdossier_overview=subdossier.unrestrictedTraverse('tabbedview_view-overview')
        ...
        self.assertEquals(
            [[box.get('id') for box in boxes] for boxes in subdossier_overview.boxes()],
            [
                ['newest_tasks',],
                ['participants',],
                ['newest_documents', 'description']
        ...
        self.assertEquals(
            subdossier_titles,
            ['Subdossier 2','Subdossier 5','Subdossier 1','Subdossier 6','Subdossier 3'])
        ...
        self.assertEquals(
            document_titles,
            [u'Document 10', u'Document 8', u'Document 12', u'Document 2', u'no_subject', u'Document 5', u'Document 1',  u'no_subject', u'Document 6', u'Document 11', ])

        # check description
        dossier.description = 'Simple test description'
        self.assertEquals(overview.description(), 'Simple test description')

        # check participation box (should include the responsible of the dossier)
        [s.title for s in  overview.sharing()]

    def _create_objects(self, context, type, title, number):
        for i in range(1, number+1):
            context.invokeFactory(
                type,'%s-%s' % (title.lower(), i),
                title='%s %s' % (title, i))
