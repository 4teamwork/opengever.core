...
class DocuComposerWizardForm(DexterityExtensibleForm, form.AddForm):
    ...
    def updateWidgets(self, *args, **kwargs):
        ...
        # set paper_form temporary to True and hide it. 
        # So it's possible to evade the validator.
        self.groups[0].fields['paper_form'].mode = interfaces.HIDDEN_MODE
        self.groups[0].fields['paper_form'].field.default = True
...
class CreateDocumentWithFile(grok.CodeView):
    ...
    def render(self):
        ...
        if data:
            ...
            # set paper_form to false because a digital file is generated by Docucomposer 
            data['paper_form'] = False

            new_doc = createContentInContainer(dossier,
                                               'opengever.document.document',
                                               **data)
            mimetype = mimetypes.types_map[filename[filename.rfind('.'):].lower()]

            fields = dict(schema.getFieldsInOrder(IDocumentSchema))
            fileObj = fields['file']._type(data=uploadFile, contentType=mimetype, filename=filename)
