...
class BaseDocumentMixin(object):
    ...
    def is_inside_a_closed_dossier(self):
        dossier = self.get_parent_dossier()
        if not IDossierMarker.providedBy(dossier):
            return False

        return not dossier.is_open()

    def is_inside_a_template_folder(self):
    ...
    def is_movable(self):
        return \
            not self.is_inside_a_task() and \
            not self.is_inside_a_proposal() and \
            not self.is_inside_a_closed_dossier() and \
            not ILockable(self).locked()
