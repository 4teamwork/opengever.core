...
class TestWebActionsPost(IntegrationTestCase):
    ...
    @browsing
    def test_creating_action_with_same_unique_id_fails(self, browser):
...
class TestWebActionsPatch(IntegrationTestCase):
    ...
    @browsing
    def test_updating_action_with_same_unique_id_fails(self, browser):
        self.login(self.webaction_manager, browser=browser)
        ...
        url = '%s/@webactions/%s' % (self.portal.absolute_url(), action_to_update['action_id'])

        action_delta = {'unique_name': existing_action['unique_name']}

        with browser.expect_http_error(code=400, reason='Bad Request'):
            browser.open(url, method='PATCH', data=json.dumps(action_delta),
                         headers=self.HEADERS)
