...
class PropertySheetsGet(Service):
    ...
    GET http://localhost:8080/fd/@propertysheets/question HTTP/1.1
    """
    ...
    def reply_list_all_sheets(self):
        ...
        result = {"@id": base_url, "propertysheets": []}
        for schema_definition in storage.list():
            sheet_definition = {"@id": "{}/{}".format(base_url, schema_definition.name)}
            result["propertysheets"].append(sheet_definition)
        return result
