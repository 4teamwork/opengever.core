...
class TestContentStatsIntegration(IntegrationTestCase):
    ...
    def test_checked_out_docs_stats_provider(self):
        ...
        self.assertEqual({'checked_out': 0, 'checked_in': 38}, stats_provider.get_raw_stats())

        self.checkout_document(self.document)
        self.assertEqual({'checked_out': 1, 'checked_in': 37}, stats_provider.get_raw_stats())
    ...
    def test_checked_out_docs_stats_provider_in_view(self, browser):
        ...
        self.assertEqual([['', 'checked_in', '38'], ['', 'checked_out', '0']], table.lists())

        self.checkout_document(self.document)
        browser.open(self.portal, view='@@content-stats')
        table = browser.css('#content-stats-checked_out_docs').first
        self.assertEqual([['', 'checked_in', '37'], ['', 'checked_out', '1']], table.lists())
