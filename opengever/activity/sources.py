...
def get_regular_watcher_ids(context):
    """Returns all regular watcher ids for the given contxt
    """
...
class PossibleWatchersSourceUsers(AssignedUsersSource):
    ...
    def _filter_not_subscribing_watchers(self, query):
        ...
        """
        query = query.filter(User.userid.notin_(get_regular_watcher_ids(self.context)))
        return query
...
class PossibleWatchersSourceGroups(AllFilteredGroupsSourcePrefixed):
    ...
    def _filter_not_subscribing_watchers(self, query):
        ...
        group_ids = [watcher.split(':', 1)[-1] for watcher
                     in get_regular_watcher_ids(self.context)
...
class PossibleWatchersSourceForUsersTeamsGroups(BaseMultipleSourcesQuerySource):
...
def can_manage_groups(context):
    is_feature_enabled = api.portal.get_registry_record(
        'is_group_watchers_feature_enabled', interface=IBaseSettings)
    has_permission = api.user.has_permission('opengever.activity: Manage Groups as Watchers',
                                             obj=context)
    return is_feature_enabled and has_permission


def get_possible_watchers_source(context):
    if can_manage_groups(context):
        return PossibleWatchersSourceForUsersTeamsGroups(context)
    return PossibleWatchersSourceUsers(context)
