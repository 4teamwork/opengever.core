...
class TestRisProposalStateDecided(IntegrationTestCase):

    features = ('ris',)

    def setUp(self):
        super(TestRisProposalStateDecided, self).setUp()
        with self.login(self.manager):
            self.ris_proposal = create(
                Builder('ris_proposal')
                .within(self.dossier)
                .having(document=self.document)
                .in_state('proposal-state-decided')
            )
    ...
    def test_visible_actions_for_dossier_responsible(self, browser):
        ...
        self.assertEqual(editbar.menus(), [])
