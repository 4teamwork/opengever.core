...
class TestGenericSetupUninstall(TestCase, GenericSetupUninstallMixin):
    ...
    package = 'opengever.theme'
    is_product = True
    skip_files = (
        # Because the order of the installed mimetypes may be different in the
        # before and after run.
        'mimetypes.xml',

        # The registry.xml was changed by p.a.theming on installation and
        # contains configuration of our theme.
        # When uninstalling the theme, we usally want to install another theme
        # before.
        # Resetting the registry.xml would uninstall the new theme.
        'registry.xml',
    )
    ...
    def assertSnapshotsEqual(self, before_id='before-install',
        ...
        for index, line in enumerate(diff_list):
            if line.startswith('-') and '<value/>' in line and \
                diff_list[index+1].startswith('+') and '<value></value>' in \
                    diff_list[index+1]:
