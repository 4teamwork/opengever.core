        ...
        if 'paths' in self.request.keys():
            ...
            for path in paths:
                ...
                if not obj.cb_isCopyable():
                    raise CopyError('An Item can not be copied')
                objlist.append(obj)
            for objid in objlist:
                cplist.append(aq_parent(aq_inner(objid)).manage_copyObjects(objid.id))
            cpstring = ':'.join(cplist)
            resp=self.request['RESPONSE']
            resp.setCookie('__cp', cpstring, path='%s' % self.cookie_path(self.request))
